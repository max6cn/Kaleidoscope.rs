...
impl <'a> Parser {
        ...
        if let Token::tok_identifier(idName) = self.currentToken.clone() {
            ...
            if self.currentToken.clone() != Token::tok_char(')') {
                loop {
                    ...
                    if self.currentToken.clone() == Token::tok_char(')') { break ;}
                    if self.currentToken.clone() != Token::tok_char(',') {
            ...
            // eat the ')'
            self.getNextToken();
            let callast = CallExprAst{Callee: idName,Args: args};
    ...
    fn parseNumberExpr(&mut self) -> Option<&'a ExprAst> {
        ...
        let result = NumberExprAst{val:
                                   match self.currentToken.clone() {
                                       Token::tok_number(v) => v ,
                                       _ => {debug!("Invalid numebr");0.0}}};
    ...
    fn parsePrimary(&mut self) -> Option<&'a ExprAst> {
        ...
        match self.currentToken.clone() {
            ...
            _ => {debug!("unknow token when expecting an expression"); None}
        }
    }
    ///----------------------------------------------------------------------///
    /// binoprhs
    ///   ::= ('+' primary)*
    ///----------------------------------------------------------------------///
    fn parseBinOpRHS(&mut self,exprPrec :i8, lhs : &'a ExprAst)
                     -> Option<Box< &'a ExprAst>>{
                ...
                Some(rhs) => {
                    ...
                    if tokPrec < nextPrec {
                        match self.parseBinOpRHS(tokPrec+1,rhs){
                            Some(rhs) => {
                                let rhs = *rhs;
                                let lhs = BinaryExprAst{Op: binOp,LHS:lhs,RHS:rhs} ;
                                debug!("Got Expr: {:?}",&lhs);
                                return Some(Box::new(
                                    unsafe { transmute(&lhs) }
                                    )) ;}
                            None => {return None;}}}},
                None => {return None; }}}
    }
                    ...
                    Token::tok_char('(') => {
                        ...
                        match self.currentToken  {
                            Token::tok_char(')') => {
                                // success
                                self.getNextToken(); //eat ')'
                                let proto = PrototypeAst{Name: FnName,Args: argname};
                                debug!("Got prototype: {:?} ",&proto);
                                Some(proto)},
                            _ => { debug!("Expecting ( in prototype");None } } } ,
                    _ => { debug!("Expecting ( in prototype");None}}},
            _ => { debug!("Expecting function name in prototype");None } }
    }
    ///----------------------------------------------------------------------///
        ...
        match self.parseProtoType() {
            Some(proto) => {
                match self.parseExpression() {
                    Some(expr) => Some(Box::new(FunctionAst{Proto:proto,Body:&expr})),
                    None => None}},
            None => None}
    }
    ///----------------------------------------------------------------------///
    ...
    fn parseTopLevelExpr(&mut self) -> Option<Box<FunctionAst>> {
        if let Some(expr) = self.parseExpression() {
            ...
            let prot1 = Box::new(PrototypeAst{Name:"".to_string(), Args: vec![]});
            //let proto  :&'static PrototypeAst =&  prot1;
            return Some(Box::new(FunctionAst{Proto: *prot1,Body:*expr}));
        }
        None
    }
    ...
    fn handleDefinition(&mut self) {
        match self.parseDefinition() {
            Some(_) => {debug!("Parsed a function Definition");},
            None => {self.getNextToken();},
        }
    }
    fn handleExtern(&mut self) {
        match self.parseExtern() {
            Some(_) => {debug!("Parsed an extern \n");},
            None => {self.getNextToken();},
        }
    }
    fn handleTopLevelExpr(&mut self) {
        match self.parseTopLevelExpr() {
            Some(_) => {debug!("Parsed an top-level expr");},
            None => {self.getNextToken();},
        };
    }
    pub  fn run (&mut self) -> (){
        print!("ready> ");   stdout().flush().unwrap();
        self.getNextToken();
        loop {
            let token = self.currentToken.clone();
            match token{
                Token::tok_eof       => { return;},
                Token::tok_char(';') => {self.getNextToken();continue},
...
mod test {
    ...
    fn test_def() {
        ...
        assert!( match res { None => false, _ => true}); // test case wrong
    }
}
